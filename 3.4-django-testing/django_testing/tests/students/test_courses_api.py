...
def test_get_first_course(client, course_factory):
    ...
    assert expected[0].id == data[0]['id']
...
def test_update_course(client, course_factory):
    course_factory(_quantity=10)
    response_put = client.put('/api/v1/courses/', data={'id': 2, 'name': 'test_create_course'})
    response_get = client.get('/api/v1/courses/')
    expected = Course.objects.get(id=2)
...
def test_delete_course(client, course_factory):
    course_factory(_quantity=10)
    response_delete = client.delete('/api/v1/courses/', data={'id': 2})
    response_get = client.get('/api/v1/courses/')
